...
export default function Cart() {
  const cartFeature = useFeature(queryBuilder().getStore_cart());
  const router = useRouter();

  if (cartFeature == "OFF") return <></>;
  ...
  const shippingRate = subTotalPrice > 0 ? 5 : 0;
  const tax = (subTotalPrice + shippingRate) * taxRate;
  const totalPrice = subTotalPrice + shippingRate + tax;

  const handleEnterCheckout = () => {
    cartFeature?.signalEnterCheckout({
      cartSize: totalItems,
